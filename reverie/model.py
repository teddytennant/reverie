"""Decoder-only transformer core, in Equinox.

Two differences from the usual LM module, both there so the caller can reason in
latent space. ``__call__`` takes input embeddings ``[T, D]`` instead of token
ids, so a sequence can mix token embeddings with continuous thought vectors. And
it returns the post-final-norm hidden states next to the logits, since
``hidden[-1]`` is what gets fed back as the next input embedding.

One unbatched sequence at a time; batch with ``jax.vmap``.
"""
...
class ModelConfig:
    ...
    n_layers: int = 6
    n_heads: int = 4
    d_ff: int | None = None  # default is ~8/3 * d_model, rounded up to a multiple of 64
    max_seq_len: int = 512
...
def _rope_cos_sin(
    ...
    # GPT-NeoX layout: rotate halves rather than interleaved pairs
...
class Transformer(eqx.Module):
    ...
    Returns ``(logits, hidden)``; ``hidden`` is the post-final-norm last-layer
    state, the same tensor the LM head reads.
    """
    ...
    def backbone(
        ...
        ``attn_mask`` is an additive [T, T] bias, 0 to attend and -inf to block;
        None means a plain causal mask. Latent reasoning passes causal AND
        validity so nothing ever attends to a pad column.
        """
        t = x.shape[0]
