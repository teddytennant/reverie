"""Tokenizing and batching ProsQA-style latent-planning instances.

The Rust generator in `data-gen/` emits the abstract problem as JSONL:
    {id, entities:[name...], edges:[[a,b]...], source, candidates:[c0,c1],
     answer, gold_path:[...], n_hops, n_distractors}

Everything textual happens here. The vocabulary is word-level and concept names
are atomic tokens, so there is no BPE. Each instance renders to token ids for
one of three supervision modes:

  nocot: prompt [facts, query, <bot>], scored on the answer token only.
  cot: prompt, then the gold steps, then the answer; CE on steps and answer.
  latent: prompt only. The model appends K continuous thoughts itself and reads
    one answer token. We carry the gold path along for trajectory distillation
    and ``n_hops`` for halt supervision.

The answer being a single concept token is why reverie/latent.py can read out
every depth in one matmul.
"""
...
# --- concept names. data-gen/src/main.rs has its own concept_name; the two have
# to produce the same string for the same index or the ids won't line up.
...
def render(inst: dict, vocab: Vocab) -> Rendered:
    ents = inst["entities"]
    cid = lambda i: vocab.tok2id[ents[i]]  # entity index to token id

    # "every A is a B ." per edge. The generator already shuffled the edges and
    # its order is deterministic, so don't reshuffle here.
    facts: list[int] = []
    for a, b in inst["edges"]:
        facts += [vocab.tok2id["every"], cid(a), vocab.tok2id["is"],
                  vocab.tok2id["a"], cid(b), vocab.tok2id["."]]

    # query: <q> is SOURCE a C0 or C1 .
    ...
    # trajectory-distillation targets: the concept reached after each hop, so
    # thought t decodes to path[t].
...
def global_lengths(insts: list[dict], vocab: Vocab) -> tuple[int, int]:
    """Max prompt length and max CoT length over a dataset, so every batch can
    pad to one shape and JAX compiles once."""
...
def collate(insts: list[dict], vocab: Vocab, max_steps: int,
    ...
    Pass the dataset-wide ``prompt_len``/``cot_len`` from global_lengths so all
    batches share shapes; otherwise JAX recompiles whenever a batch happens to
    ...
    for b, x in enumerate(r):
        # Left-pad, so every prompt ends at column Sp-1 no matter its length.
        # The K thought slots then sit at columns Sp..Sp+K-1 for every example
        # in the batch, which is what lets latent_unroll index them by position
        # instead of tracking a per-example offset.
        lp = len(x.prompt)
