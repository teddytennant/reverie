#!/usr/bin/env bash
# Phase-0: multi-hop reachability over clean is-a chains (branch=0) at depths
# 2-4, small model trained from scratch. Runs the five-method comparison, the
# two objective ablations and the branching regime, then prints the tables.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# harder "search" regime: distractor branches and traps
