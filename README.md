...
**Honest caveat (probed, not hand-waved).** Latent reasoning gives **no accuracy advantage at this scale**: on the search task **No-CoT reaches 0.847 with zero reasoning steps**, matching Reverie's 0.850. We suspected a component-membership shortcut and tested it — adding decoy→source cross-edges (`--connect`) to weakly-connect the graph barely moved No-CoT (0.847 → 0.830 → 0.803 for connect 0/6/12). The real reason: a 2-layer transformer just solves directed reachability over these small graphs in one forward pass, so multi-step reasoning isn't *needed* for accuracy. Showing an advantage needs deeper problems + GPT-2 scale (future work). What no shortcut or one-pass solver can explain is the **calibration** — the halt learns to spend exactly `n_hops` steps, a fact about problem *depth*, not the answer. See [`docs/paper.md`](docs/paper.md) §6. Reproduce: `make phase0`.
